...
--  P_FECHA_EJECUCION       ASIGNADA EN RUNTIME
    ...
    BEGIN
        ...
        -- FIND CUENTAS
        SELECT CUENTA_ID INTO CUENTA_1 FROM CUENTA WHERE IBAN LIKE IBAN_1 FOR UPDATE; -- NO SE PERMITE INSERTAR O ELIMINAR LA CUENTA DURANTE TRANSACCIÓN
        SELECT CUENTA_ID INTO CUENTA_2 FROM CUENTA WHERE IBAN LIKE IBAN_2 FOR UPDATE; -- NO SE PERMITE INSERTAR O ELIMINAR LA CUENTA DURANTE TRANSACCIÓN

        -- SI AMBAS CUENTAS NO SE ENCUENTRAN EN LA BD, LA TRANSACCIÓN ES INVÁLIDA
        IF CUENTA_1 IS NULL AND CUENTA_2 IS NULL THEN
            RAISE CUENTAS_NO_EXISTEN_EXCEPTION;
        END IF;
        
        -- SE CONVIERTE A LA DIVISA DE LA CUENTA
        SELECT CAMBIO_EURO INTO CAMBIO_1 FROM DIVISA WHERE ABREVIATURA = P_DIVISA_ABREVIATURA FOR UPDATE; -- NO SE PERMITE ACTUALIZAR EL CAMBIO DURANTE TRANSSACCIÓN
        SELECT CAMBIO_EURO INTO CAMBIO_2 FROM DIVISA WHERE ABREVIATURA = P_DIVISA_ABREVIATURA2 FOR UPDATE;-- NO SE PERMITE ACTUALIZAR EL CAMBIO DURANTE TRANSSACCIÓN
        ...
        IF CUENTA_1 IS NULL THEN -- CUENTA_1 ESTÁ REGISTRADA
            -- SE COMPRUEBA QUE LA OTRA CUENTA SEA FINTECH
            -- NO ES NECESARIO BLOQUEAR LAS CUENTAS FINTECH, SEGREGADAS O POOLED YA QUE CUENTA ESTÁ BLOQUEADA
            ...
            IF FINTECH IS NULL THEN
                ...
                ELSE -- LA CUENTA FINTECH ES SEGREGADA
                    ...
                    -- AUMENTAR SALDO
                    -- CUENTA_REFERENCIA SÍ ES NECESARIO BLOQUEARLA YA QUE EL SALDO SE PODRÍA MODIFICAR DURANTE TRANSACCIÓN
                    UPDATE CUENTA_REFERENCIA SET SALDO = SALDO + IMPORTE WHERE CUENTA_CUENTA_ID = REF_AUX FOR UPDATE;
                END IF;
            ELSE -- LA CUENTA FINTECH ES POOLED
                ...
                ELSE -- CUENTA POOLED YA TIENE REF CON DIVISA DESEADA
                    ...
                    -- CUENTA_REFERENCIA SÍ ES NECESARIO BLOQUEARLA YA QUE EL SALDO SE PODRÍA MODIFICAR DURANTE TRANSACCIÓN
                    UPDATE CUENTA_REFERENCIA SET SALDO = SALDO + IMPORTE WHERE CUENTA_CUENTA_ID = REF_AUX FOR UPDATE;
                    -- DEPOSITAR_EN SÍ ES NECESARIO BLOQUEARLA YA QUE EL SALDO SE PODRÍA MODIFICAR DURANTE TRANSACCIÓN
                    UPDATE DEPOSITAR_EN SET SALDO = SALDO + IMPORTE WHERE CUENTA_REF_ID = REF_AUX AND POOL_ID = FINTECH FOR UPDATE;
                END IF;
        ...
        ELSIF CUENTA_2 IS NULL THEN -- CUENTA_2 NO ESTÁ REGISTRADA
            ...
            ELSE -- LA CUENTA FINTECH ES POOLED
                ...
                IF DIVISA_R IS NULL OR REF_AUX IS NULL THEN -- DIVISA NOT FOUND -> EXCEPTION
                    -- NO SE PUEDE CREAR UNA NUEVA CUENTA REFERENCIA YA QUE EL SALDO SERÍA 0
                    -- AQUÍ LA CUENTA POOLED ES LA EMISORA DE LA TRANSACCIÓN LUEGO EL SALDO NO PUEDE SER 0
                    RAISE DIVISA_NOT_FOUND_EXCEPTION;
                ELSE -- CUENTA POOLED YA TIENE REF CON DIVISA DESEADA
        ...
        ELSE -- TODAS LAS CUENTAS ESTÁN REGISTRADAS
