  ...
  BEGIN
	...
	ELSE
		IF P_IDENTIFICACION IS NOT NULL THEN
			SELECT COUNT(CLIENTE_IDENTIFICACION) INTO AUX FROM CLIENTE WHERE CLIENTE_IDENTIFICACION LIKE P_IDENTIFICACION;
			IF AUX > 0 THEN
...
ID_AUX              PERSONA_AUTORIZADA.ID%TYPE AS
  BEGIN
...
P_ID_EMPRESA IN AUTORIZACION.EMPRESA_ID%TYPE) AS
  BEGIN
    -- TAREA: Se necesita implantación para PROCEDURE PK_GESTION_CLIENTES.DEL_AUTORIZADO
    NULL;
  END DEL_AUTORIZADO;
