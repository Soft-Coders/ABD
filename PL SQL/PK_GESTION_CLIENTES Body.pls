...
create or replace PACKAGE BODY PK_GESTION_CLIENTES AS
    ...
    BEGIN
        ...
        --Se aprovecha la variable para asignar el ID seg�n la secuencia SQ_CLIENTE
    ...
    BEGIN
        ...
        ELSE
            -- La identificaci�n solo se actualiza si no existe en la base de datos
            -- En caso de que la identificaci�n no se haya cambiado se mantendr� la misma y no se actualizar�
            -- En caso de que se quiera cambiar la identificaci�n por una que ya tiene otro cliente se lanzar� un ERROR
            -- En caso de que la identificaci�n se quiera cambiar por una que no tenga ning�n cliente se actualizar� el valor en la tabla
    ...
    */
    ...
    BEGIN
        ...
        END IF;
        IF P_TIPO <> 'CONSULTA' AND P_TIPO <> 'OPERACI[�O]N' THEN
            RAISE DATOS_INCORRECTOS_EXCEPTION;
        ELSE
    ...
    BEGIN
        ...
        SELECT ID INTO EMPRESA_ID_AUX FROM CLIENTE WHERE ID LIKE P_EMPRESA_ID FOR UPDATE; -- SE BLOQUEA FILA (NO SE PERMITE A�ADIR EMPRESAS CUANDO SE VA A DAR DE BAJA)
        ...
        SELECT PERSONA_AUTORIZADA_ID INTO CONT_AUT_EMP FROM AUTORIZACION WHERE PERSONA_AUTORIZADA_ID LIKE PERSONA_AUTORIZADA_ID_AUX FOR UPDATE; -- SE BLOQUEA FILA, NO A�ADIR AUOTIRIZACIONES DURANTE BAJA
    ...
    BEGIN
        ...
        END IF;
            ...
            SET 
                TIPO = P_TIPO,
                -- Consideramos que la empresa no deber�a  
        ...
        --Quisimos hacerlo de esta manera pero creemos que nos puede complicar la l�gica del proyecto en un futuro, por lo que hemos decidido apartarla
        /*
        IF P_IDENTIFICACION IS NOT NULL THEN
