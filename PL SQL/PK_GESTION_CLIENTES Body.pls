...
P_ID_EMPRESA IN AUTORIZACION.EMPRESA_ID%TYPE)
 ...
 ID_AUX              PERSONA_AUTORIZADA.ID%TYPE;
  ...
  PROCEDURE BAJA_AUTORIZADO(P_ID_AUTORIZADO               IN AUTORIZACION.PERSONA_AUTORIZADA_ID%TYPE,
    P_EMPRESA_ID                   IN  AUTORIZACION.EMPRESA_ID%TYPE) IS
  ...
  BEGIN
    ...
    SELECT ID INTO PERSONA_AUTORIZADA_ID_AUX FROM PERSONA_AUTORIZADA WHERE ID LIKE P_ID_AUTORIZADO;
...
AUX INTEGER;
  BEGIN
    ...
    EXCEPTION    
    WHEN AUTORIZADO_NO_EXISTENTE_EXCEPTION THEN
    -- Para que funcione este método debo de escribir SET SERVEROUTPUT ON; justo antes de lanzar este método
    DBMS_OUTPUT.PUT_LINE('NO EXISTE EL AUTORIZADO');
